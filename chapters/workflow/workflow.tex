...
\begin{figure}
	...
	\end{subfigure}
	\caption[Variation in the iterative Hegyi index with parameters]{Variation in the iterative Hegyi index with: increasing diameter ($D_{j}$) of 12 equally distant stems (a), increasing distance ($L_{ij}$) of 12 stems of equal stem diameter (b), increasing number ($n$) of equally distant stems with equal diameter (c), and increasing variance in stem diameter ($\sigma{}D_{j}$), measured by drawing 12 random samples from a log normal distribution with increasing standard deviation, and a mean of 5, repeated 100 times (d).}
