...
% A key supposition in \autoref{ch:befr} was that positive biodiversity effects may be diminished due to the actions of disturbance by fire and herbivory, which reduce stem density and AGB, thus reducing the structuring effect of competition. We found that disturbance by fire reduced stem density, which in turn reduced AGB, but that disturbance actually had a positive effect on species diversity. Additionally, it was found that there was a stem density threshold below which positive biodiversity effects were diminished. 
...
% Abundance evenness is as important as species richness (chapter BEFR). It's not only the number of species you have, but also their relative evenness. Normally, you would expect wooded ecosystems to achieve something like a Fisher's alpha curve of species abundance, with a long tail of rare species. In this analysis it appears that greater evenness maximises ecosystem function. 
% What do other studies say about this?
% High abundance evenness isn't necessarily the "natural" state.
% Is abundance evenness increased by disturbance, or decreased?
% Evenness caused a decrease in cumulative EVI, growing season length, and green-up lag. This is coz evenness is associated with a reduction in the dominance of large canopy-forming tree species which dominate the land-surface phenology signal  and have access to deep groundwater reserves (e.g. \textit{Brachystegia} spp. and \textit{Julbernardia} spp.). The detarioideae species drive the pre-rain green-up? A place for selection effects depending on the ecosystem function being studied
% In cluster2, which weasd not dominated by detarioideae species, evenness did cause an increase in cuimulative evi and season length
...
% Land-surface phenology plays an important regulating role in the global carbon water and nitrogen cycles \citep{Richardson2013}. 
% Land-surface phenology is a proxy for productivity?
% In southern African woodlands particularly, much interest has been paid to the phenomenon of pre-rain green-up. 
% Conservative species intitiate leaf production before the wet season has commenced
...
% Larger trees didn't have a strong effect on pre-rain green-up, but did delay senescence,

% Senescence is not controlled by trees, maybe by grass \citep{}.

% Incorporating predictions of biotic change into carbon cycling models has been limited (Ahlstrom et al., 2015; Bodegom et al., 2011), owing to large uncertainties in the effects of diversity on Gross Primary Productivity (GPP). Our study provides a link by demonstrating a strong positive relationship between species diversity and cumulative EVI, which itself correlates with GPP (Sjöström et al., 2011).
...
% Weaker effects than expected. Lots of stochasticity in canopy structure caused by small spatial scale environmental heterogeneity, and landscape history. 
...
% Unlike in \autoref{ch:befr}, tree size variation did not have a large effect on canopy complexity, but species diversity did influence both tree size variation and canopy complexity separately. This suggests that there are species-specific differences in the plasticity and physiological limits of crown shape that are independent of tree size
